...
export async function GET(req: NextRequest) {
  try {
    ...
    const category = searchParams.get("category"); // Fixed: was incorrectly set to "city"
    const day = searchParams.get("day"); // Get the 'day' query parameter

    // Individual space query
    if ((id && id !== "null") || (slug && slug !== "null")) {
      const query = id && id !== "null" ? { _id: id } : { slug: slug };
    ...
    if (city && city !== "null") filter.city = city;
    if (category && category !== "null") filter.category = category;

    if (day) {
      filter.operatingHours = { $elemMatch: { day, status: "Open" } };
    }
    console.log(filter);
    // Get total count with filter
    const totalSpaces = await Space.countDocuments(filter);
